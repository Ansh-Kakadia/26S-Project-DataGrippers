...
REPORTS = {
    "Spring 2026": {
        ...
        "no_show_rate": {
            "Soccer": 15, "Basketball": 20, "Volleyball": 11, "Hockey": 29,
        },
        "budget": {
            "Soccer": 8000, "Basketball": 5120, "Hockey": 2200, "Volleyball": 5580,
        },
    ...
    "Fall 2025": {
        ...
        "no_show_rate": {
            "Soccer": 18, "Basketball": 22, "Volleyball": 13, "Hockey": 25,
        },
        "budget": {
            "Soccer": 7600, "Basketball": 4800, "Hockey": 2100, "Volleyball": 4900,
        },
    ...
    "Summer 2025": {
        ...
        "no_show_rate": {
            "Soccer": 20, "Basketball": 17, "Volleyball": 9, "Hockey": 31,
        },
        "budget": {
            "Soccer": 7800, "Basketball": 4500, "Hockey": 2300, "Volleyball": 4200,
        },
        "budget_change_pct": -5,
    },
}
...
def budget_chart(sports, budgets, total):
    ...
    fig.update_layout(
        title=dict(text="Budget Allocation per Sport", font=dict(family=FONT_MONO, size=14, color="#111"), x=0.43, xref="paper"),
        plot_bgcolor=CHART_BG,
        paper_bgcolor=CHART_BG,
        font=dict(family=FONT_MONO, size=12, color="#111"),
        ...
        height=220,
        barmode="overlay",
        xaxis=dict(range=[0, max_budget * 1.3], showgrid=True, gridcolor="#eee", showticklabels=False),
...
def show():
    ...
    st.markdown("<h2 style='font-family:monospace; margin-bottom:2px;'>Intramural Activity Report</h2>", unsafe_allow_html=True)
    col_label, col_select, _ = st.columns([0.01, 2, 3])

    with col_select:
        selected = st.selectbox("report_select", list(REPORTS.keys()), label_visibility="collapsed")
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-family:monospace; color:#555; margin-bottom:16px;'>{selected} Report</div>", unsafe_allow_html=True)
